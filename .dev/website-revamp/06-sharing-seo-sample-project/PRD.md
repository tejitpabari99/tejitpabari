...
**Confirmed load-bearing for CI specifically, not just tidy practice.** Now that the owner has adopted CI (brief §4's amendment) and `npm run build` runs inside SP08's GitHub Actions runners on every PR and every push to `main` (`08-ci-deploy-pipeline` §4.2–§4.4), the "committed, not fetched" choice above stops being a nice-to-have and becomes a build-correctness requirement: a fresh CI checkout has no ambient font cache, no prior `fc-cache` state, nothing a browser's own font-loading machinery could fall back on — and a GitHub Actions runner reaching out to Google Fonts (or anywhere else) at build time to fetch Montserrat would make every single deploy depend on a third-party network call succeeding, exactly the kind of build-breaking external dependency this project avoids everywhere else. Because the three `.ttf` files are already committed under version control rather than downloaded, `generate-og-cards.mjs`'s `readFileSync(path.join(fontDir, ...))` calls (above) resolve identically whether the build runs on the owner's laptop or a clean, disposable CI runner that has never seen this repo before — no CI-specific accommodation was needed, this design was already correct for that environment before CI existed to need it.
...
This closes the loop brief §5 opens without contradicting SP04's own explicit scope boundary: the test *proves* the coupling exists and that a broken hosted page is caught immediately and named precisely (not "scoped by tooling," since SP04 declined to build isolation tooling) — which is exactly the honest outcome SP04 §4.9 already predicted ("a broken hosted page still blocks the whole site's build until fixed, same as today"). Tracked as a one-time manual verification in §8, not built as a recurring automated test — **though the underlying coupling this verification proves is, since CI's adoption, also exercised automatically on every real PR**: SP08's CI pipeline (`08-ci-deploy-pipeline` §4.2) runs `npm run build` on every pull request, so any future hosted `/live` page that actually breaks the build fails that PR's CI run and blocks its merge, without anyone needing to deliberately break something on a scratch branch to find out. This item stays a one-time manual exercise specifically to *prove* that mechanism works as designed once, deliberately, before trusting it silently — not because no automated equivalent exists.
...
- **A real rendered-output smoke test**: `generate-og-cards.mjs`'s `main()` run once against the real `src/content/{projects,research}` directories produces exactly one PNG per content file plus `default.png`, each readable and reporting `1200×630` when its own header bytes are parsed — the actual proof the pipeline produces valid images, not just correct JSX. Run locally during implementation, and exercised again automatically on every real PR/merge once SP08's CI pipeline runs `npm run build` (which triggers this generator via `prebuild`, §4.3) against a clean checkout — see §4.3's confirmation that the vendored-font design already handles that clean-checkout case correctly.
...
2. **Delete the sample-project scaffolding once share previews are confirmed working:** `src/content/projects/sample-project.md`, `src/pages/live/sample-project.tsx`, `src/pages/live/sample-project.test.tsx`, and its one-line entry in `src/pages/live/registry.ts`'s `HOSTED_LIVE_PAGES`. `npm run check:launch` (SP02 §4.9) will confirm the `demo: true` count drops from 1 to 0 once this is done — treat that as the actual completion signal, not just "I remember deleting it."
...
- `[RESOLVED: confirmed the `prebuild` OG-card/sitemap generators run correctly in a clean CI checkout]` — re-checked specifically once the owner adopted CI (brief §4's amendment, SP08 `08-ci-deploy-pipeline`): the Montserrat TTFs satori needs (§4.3) were already committed to the repo under `scripts/assets/fonts/`, not fetched from Google Fonts or anywhere else at build time — this PRD's original design happened to already be CI-safe, since a CI runner has no ambient font cache and a network dependency at build time would otherwise be a build-breaking dependency on a third party. Nothing needed changing; §4.3 now states this explicitly rather than leaving it implicit in "why vendor the fonts at all."
