...
> **Amended 2026-08-31 (correction):** the amendment directly above is factually wrong on one point and is corrected here rather than rewritten — **no Firebase-scaffolded integration was ever actually installed on this repo.** Verified directly and independently (SP08 §1): `git ls-tree -r <branch> --name-only | grep '^\.github/workflows'` returns nothing on `main`, `website-revamp`, or either other remote branch; `gh secret list` and `gh variable list` both return empty; no CI-shaped service account exists in the `tejitpabari-99` project's IAM. The owner's belief that the integration had run (possibly against a different repo, or abandoned mid-flow) does not match what's actually on `github.com/tejitpabari99/tejitpabari`. **The underlying decision — adopt CI — still stands and is unaffected**; only the premise that there was an existing scaffold to *reconcile* was wrong. SP08 (`08-ci-deploy-pipeline`) hand-authors both workflow files from nothing, rather than editing a generated starting point. See SP08 §1/§9, and the corrected SP01 §4.9/§9 (which previously repeated this same mistaken belief and is fixed in the same pass).
