...
| `02-content-pipeline` | Content Pipeline | Four content collections (Projects, Research, Work Experience, Legal) — on-disk layout, frontmatter contracts, `import.meta.glob` + `gray-matter` loaders, hand-rolled build-time validation (including the Nav/Footer href cross-check), `src/config/featured.ts`, the `DRAFT_DATE` placeholder mechanism, and the `/projects/<slug>/live` redirect-vs-hosted signal. | SP01 | 🟡 Partial (10/15) |
...
| `05-legal-analytics` | Legal Pages & Analytics | Obfuscated-email contact pattern; `ConsentContext` + consent banner; GA4 (`trackEvent`, the five-event `AnalyticsEventName` catalogue); `/privacy` and `/terms` drafted copy. | SP01 | 🟡 Complete except Task 20 (19/20) |
...
- **SP02 (Content Pipeline) — partially complete.** Tasks 1–10 done (collections, loaders, validation, the pre-launch content gate). Tasks 11–15 (validator/unit test suites) not started — deliberately deferred, not blocked. See `02-content-pipeline/TASKS.md`.
- **SP05 (Legal Pages & Analytics) — complete except Task 20.** Tasks 1–19 done (contact obfuscation, consent system, GA4 analytics, `/privacy` and `/terms` pages, and their unit/smoke tests). Task 20 (a `dist/` build audit) is not started — it requires a full `npm run build` with no concurrent agents in the repo, deliberately deferred rather than run mid-flight. See `05-legal-analytics/TASKS.md`.
