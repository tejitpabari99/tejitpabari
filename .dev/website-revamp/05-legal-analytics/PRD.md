...
- A "no forms" enforcement mechanism wired into `npm run build` itself (a hard CI-style gate) — this project has no CI (brief non-goal), so the mechanism SP04 ultimately builds (`npm run check:no-forms`, §4.7, §9) is a precise, manually-run script, not a build-blocking check. That script's design is SP04's, not built here.
...
- **The GitHub Actions build must supply this variable** (SP01 §4.9's "Deploy pipeline" subsection) or GA silently no-ops in production — `shouldLoadGa()` below treats a missing ID exactly like dev mode, so a CI misconfiguration produces a site that looks and works fine with zero analytics, and nothing fails loudly. That's the failure mode worth naming explicitly: a silently-missing analytics ID is exactly the kind of gap that goes unnoticed for months, not a build error someone trips over on day one.
...
// routing logic)
{
  ...
  // must be revised BEFORE that project ships, not after. There is no CI
  // (brief/SP01, "no CI for a solo builder") — this is a pre-ship checklist
  // item — SP04 built a real mechanical check on top of it, though: run
  // `npm run check:no-forms` (scripts/check-no-forms.sh, PRD 04 §4.8) before
  // adding any new entry to src/pages/live/registry.ts's HOSTED_LIVE_PAGES,
  // and before every real deploy. See PRD 05 §4.7, PRD 04 §4.7/§4.8.
},
```

**The mechanical check — resolved, not honestly-scoped-but-manual any more.** The brief's own §5 originally framed this as "grep for the claim whenever a new `/live` project ships" — a manual, pre-ship habit, since a repo-wide grep for `<input`/`<form`/`<textarea` would false-positive immediately on SP04's own `SearchFilter` (which legitimately has a search `<input>` on `/projects`/`/research`, already disclosed in §4.5's copy as "not a form"). A precise, low-noise grep target needed SP04's hosted-`/live`-project directory convention to exist first — **SP04 §4.7 now settles that convention** (`src/pages/live/<slug>.tsx` + `src/pages/live/registry.ts`, with hosted pages isolated from `SearchFilter`'s own files) **and SP04 §4.8 builds the check as `scripts/check-no-forms.sh`, exposed as `npm run check:no-forms`** — a scoped `grep -rEn '<(input|form|textarea)[ >]' src/pages/live` with zero false positives, closing the loop this PRD originally flagged as SP04-blocked:

1. **The pre-ship checklist item is now backed by a real command.** Before merging a new hosted (non-redirect) `/projects/<slug>/live` page, run `npm run check:no-forms` — a nonzero exit means: grep both legal pages for the phrase `"no forms"`, re-read the "What this site does not do" / "No forms, today" sections against the new page's actual markup, and update both pages' copy + `LAST_UPDATED` before the page ships.
2. **`.env.example` is unaffected; `package.json` gains the one script.** `check:no-forms` is not wired into `npm run build` (no CI, per brief non-goals) — it's a documented, manually-run gate, same model as SP02's `check:launch`, chainable with it (SP04 §4.8: `"check:launch": "tsx scripts/check-launch-content.ts && npm run check:no-forms"`).
...
2. **Confirm `VITE_GA_MEASUREMENT_ID` is actually reaching the production build.** The value itself is committed in `.env.example` (§4.3) — nothing local to set for a normal build. What needs owner confirmation instead: SP01's GitHub Actions deploy pipeline (§4.9 there) now runs `npm run build` in CI on every push to `main`, so the variable must be supplied there — as a repository variable or secret referenced in the workflow's `env:` — or GA silently no-ops on the deployed site. `.env.local` remains available (gitignored, SP01 §4.2) for anyone who wants a different value for local dev only.
...
6. **Confirm the "no forms" fragility-guard workflow is one you'll actually follow** (§4.7) — `npm run check:no-forms` (SP04 §4.8) is a real, precise script now, but it's still a manually-run command, not a build gate, so it only catches a violation if you (or whoever ships the next hosted `/live` project) actually run it before shipping.
...
- `[RESOLVED: GA4 measurement ID is G-9NLS3NG63M, supplied via VITE_GA_MEASUREMENT_ID]` — auto-created alongside the Firebase web app the owner registered under the `tejitpabari-99` project. Committed as the default in `.env.example`; not a secret (see §4.3's reasoning). SP01's GitHub Actions deploy pipeline (SP01 §4.9) must pass it through at build time, or the shipped site silently has no analytics.
