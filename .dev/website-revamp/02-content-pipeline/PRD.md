...
```ts
// scripts/check-launch-content.ts — runs on every PR and every merge to
// main via SP08's CI pipeline (08-ci-deploy-pipeline §4.2/§4.3/§4.4), so
// a remaining DRAFT_DATE or demo marker now fails the build automatically
// rather than depending on someone remembering to run this by hand before
// a deploy — see §9 for the history of this decision.
...
Wired as `"check:launch": "tsx scripts/check-launch-content.ts"` in `package.json` — the exact script SP04 §4.8 extends to `"check:launch": "tsx scripts/check-launch-content.ts && npm run check:no-forms"` once `check:no-forms` exists. Because it imports `src/data` directly, it inherits every validation already run by the loaders and `validateInternalLinks()` (§4.5.4) for free — a broken content file fails this same command with the same file-and-field-specific error before it ever gets to the DRAFT_DATE/demo check.

**No longer only a manually-run habit.** At the time this section was first written, brief §4 excluded CI outright, so this script's only defense against "nobody actually ran it before deploying" was being one documented command named in SP01's `README.md` pre-launch checklist. That's since changed: the owner adopted CI (brief §4's amendment), and SP08 (`08-ci-deploy-pipeline` §4.2/§4.3/§4.4) runs `npm run check:launch` as a named, separate step on every pull request and on every push to `main` — a remaining `DRAFT_DATE`/`demo: true` marker now fails the build automatically, before a deploy can happen, rather than depending on the habit holding. This is, in SP08's own words (SP08 §1), "the single most valuable consequence of adopting CI" for this specific sub-project: the exact residual risk this PRD originally accepted as real but mitigated ("manual and easy to forget," §9) is now mechanically closed rather than merely documented. See §9's updated entry.
...
- `[RESOLVED: the gate is enforced by SP08's CI pipeline — no longer a manually-run script]` — supersedes this entry's original `[DEFERRED]` form, written when brief §4 still excluded CI outright and the acknowledged residual risk ("manual and easy to forget") could only be mitigated, not closed (being named in SP01's `README.md` pre-launch checklist, SP01 §4.1). The owner has since adopted CI (brief §4's amendment); SP08 (`08-ci-deploy-pipeline` §4.2–§4.4) runs `npm run check:launch` as a separate, named step on every PR and every push to `main`, so a remaining `DRAFT_DATE`/`demo: true` marker now fails the build automatically rather than depending on the script being run by hand before a deploy. This is the single most valuable consequence of adopting CI for this sub-project specifically — see SP08 §1 and §4.9 above.
