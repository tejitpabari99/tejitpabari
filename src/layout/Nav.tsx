...
export function Nav() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-4 pt-4 sm:px-6 sm:pt-5 lg:px-8">
      <div className="mx-auto flex w-fit items-center justify-center border border-teal-secondary/15 bg-cream/92 p-1.5 shadow-pill backdrop-blur-md">
                ...
                {NAV_LINKS.map((item) => {
                  ...
                  return (
                    <li key={item.href}>
                      <Link
                        to={item.href}
                        className={`block whitespace-nowrap px-2 py-1.5 text-[0.62rem] font-semibold transition sm:px-4.5 sm:py-2.5 sm:text-[0.83rem] lg:px-5 lg:py-2.5 lg:text-[0.88rem] ${
                          isActive
